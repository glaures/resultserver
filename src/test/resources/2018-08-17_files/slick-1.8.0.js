;(function (factory) {
  ...
  var Slick = window.Slick || {};Slick = function () {
    ...
    var _ = this;_.$slideTrack.find('.kick__slick-active').attr({ 'aria-hidden': 'false' }).find('a, input, button, select').attr({ 'tabindex': '0' });
    ...
    var _ = this;if (_.options.arrows === true) {
      _.$prevArrow = $(_.options.prevArrow).addClass('kick__slick-arrow');_.$nextArrow = $(_.options.nextArrow).addClass('kick__slick-arrow');if (_.slideCount > _.options.slidesToShow) {
        _.$prevArrow.removeClass('kick__slick-hidden').removeAttr('aria-hidden tabindex');_.$nextArrow.removeClass('kick__slick-hidden').removeAttr('aria-hidden tabindex');if (_.htmlExpr.test(_.options.prevArrow)) {
        ...
        if (_.options.infinite !== true) {
          _.$prevArrow.addClass('kick__slick-disabled').attr('aria-disabled', 'true');
        }
      } else {
        _.$prevArrow.add(_.$nextArrow).addClass('kick__slick-hidden').attr({ 'aria-disabled': 'true', 'tabindex': '-1' });
      }
    }
    ...
    var _ = this,
      ...
      _.$slider.addClass('kick__slick-dotted');dot = $('<ul />').addClass(_.options.dotsClass);for (i = 0; i <= _.getDotCount(); i += 1) {
        dot.append($('<li />').append(_.options.customPaging.call(this, _, i)));
      }
      _.$dots = dot.appendTo(_.options.appendDots);_.$dots.find('li').first().addClass('kick__slick-active');
    }
  };Slick.prototype.buildOut = function () {
    var _ = this;_.$slides = _.$slider.children(_.options.slide + ':not(.kick__slick-cloned)').addClass('kick__slick-slide');_.slideCount = _.$slides.length;_.$slides.each(function (index, element) {
      $(element).attr('data-slick-index', index).data('originalStyling', $(element).attr('style') || '');
    });_.$slider.addClass('kick__slick-slider');_.$slideTrack = _.slideCount === 0 ? $('<div class="kick__slick-track"/>').appendTo(_.$slider) : _.$slides.wrapAll('<div class="kick__slick-track"/>').parent();_.$list = _.$slideTrack.wrap('<div class="kick__slick-list"/>').parent();_.$slideTrack.css('opacity', 0);if (_.options.centerMode === true || _.options.swipeToSlide === true) {
      _.options.slidesToScroll = 1;
    }
    $('img[data-lazy]', _.$slider).not('[src]').addClass('kick__slick-loading');_.setupInfinite();_.buildArrows();_.buildDots();_.updateDots();_.setSlideClasses(typeof _.currentSlide === 'number' ? _.currentSlide : 0);if (_.options.draggable === true) {
      _.$list.addClass('kick__draggable');
    }
    ...
    var _ = this,
        ...
        numOfSlides,
        originalSlides,
        slidesPerSection;newSlides = document.createDocumentFragment();originalSlides = _.$slider.children();if (_.options.rows > 0) {
    ...
    var _ = this;if (_.options.dots && _.$dots !== null) {
      $('li', _.$dots).off('click.kick__slick', _.changeSlide).off('mouseenter.kick__slick', $.proxy(_.interrupt, _, true)).off('mouseleave.kick__slick', $.proxy(_.interrupt, _, false));if (_.options.accessibility === true) {
        _.$dots.off('keydown.kick__slick', _.keyHandler);
      }
    }
    _.$slider.off('focus.kick__slick blur.kick__slick');if (_.options.arrows === true && _.slideCount > _.options.slidesToShow) {
      _.$prevArrow && _.$prevArrow.off('click.kick__slick', _.changeSlide);_.$nextArrow && _.$nextArrow.off('click.kick__slick', _.changeSlide);if (_.options.accessibility === true) {
        _.$prevArrow && _.$prevArrow.off('keydown.kick__slick', _.keyHandler);_.$nextArrow && _.$nextArrow.off('keydown.kick__slick', _.keyHandler);
      }
    }
    _.$list.off('touchstart.kick__slick mousedown.kick__slick', _.swipeHandler);_.$list.off('touchmove.kick__slick mousemove.kick__slick', _.swipeHandler);_.$list.off('touchend.kick__slick mouseup.kick__slick', _.swipeHandler);_.$list.off('touchcancel.kick__slick mouseleave.kick__slick', _.swipeHandler);_.$list.off('click.slick', _.clickHandler);$(document).off(_.visibilityChange, _.visibility);_.cleanUpSlideEvents();if (_.options.accessibility === true) {
      _.$list.off('keydown.kick__slick', _.keyHandler);
    }
    if (_.options.focusOnSelect === true) {
      $(_.$slideTrack).children().off('click.kick__slick', _.selectHandler);
    }
    $(window).off('orientationchange.kick__slick.kick__slick-' + _.instanceUid, _.orientationChange);$(window).off('resize.kick__slick.kick__slick-' + _.instanceUid, _.resize);$('[draggable!=true]', _.$slideTrack).off('dragstart', _.preventDefault);$(window).off('load.kick__slick.kick__slick-' + _.instanceUid, _.setPosition);
  };Slick.prototype.cleanUpSlideEvents = function () {
    var _ = this;_.$list.off('mouseenter.kick__slick', $.proxy(_.interrupt, _, true));_.$list.off('mouseleave.kick__slick', $.proxy(_.interrupt, _, false));
  };Slick.prototype.cleanUpRows = function () {
    var _ = this,
        originalSlides;if (_.options.rows > 0) {
    ...
    var _ = this;_.autoPlayClear();_.touchObject = {};_.cleanUpEvents();$('.kick__slick-cloned', _.$slider).detach();if (_.$dots) {
      _.$dots.remove();
    }
    if (_.$prevArrow && _.$prevArrow.length) {
      _.$prevArrow.removeClass('kick__slick-disabled kick__slick-arrow kick__slick-hidden').removeAttr('aria-hidden aria-disabled tabindex').css('display', '');if (_.htmlExpr.test(_.options.prevArrow)) {
        _.$prevArrow.remove();
      }
    }
    if (_.$nextArrow && _.$nextArrow.length) {
      _.$nextArrow.removeClass('kick__slick-disabled kick__slick-arrow kick__slick-hidden').removeAttr('aria-hidden aria-disabled tabindex').css('display', '');if (_.htmlExpr.test(_.options.nextArrow)) {
        _.$nextArrow.remove();
      }
    }
    if (_.$slides) {
      _.$slides.removeClass('kick__slick-slide kick__slick-active slick-center kick__slick-visible kick__slick-current').removeAttr('aria-hidden').removeAttr('data-slick-index').each(function () {
        $(this).attr('style', $(this).data('originalStyling'));
      });_.$slideTrack.children(this.options.slide).detach();_.$slideTrack.detach();_.$list.detach();_.$slider.append(_.$slides);
    }
    _.cleanUpRows();_.$slider.removeClass('kick__slick-slider');_.$slider.removeClass('kick__slick-initialized');_.$slider.removeClass('kick__slick-dotted');_.unslicked = true;if (!refresh) {
    ...
    var _ = this;_.$slider.off('focus.kick__slick blur.kick__slick').on('focus.kick__slick blur.kick__slick', '*', function (event) {
    ...
    var _ = this,
        ...
        verticalOffset = 0,
        targetSlide,
        coef;_.slideOffset = 0;verticalHeight = _.$slides.first().outerHeight(true);if (_.options.infinite === true) {
      if (_.slideCount > _.options.slidesToShow) {
        _.slideOffset = _.slideWidth * _.options.slidesToShow * -1;coef = -1;if (_.options.vertical === true && _.options.centerMode === true) {
    ...
    if (_.options.variableWidth === true) {
      if (_.slideCount <= _.options.slidesToShow || _.options.infinite === false) {
        targetSlide = _.$slideTrack.children('.kick__slick-slide').eq(slideIndex);
      } else {
        targetSlide = _.$slideTrack.children('.kick__slick-slide').eq(slideIndex + _.options.slidesToShow);
      }
      if (_.options.rtl === true) {
        if (targetSlide[0]) {
      ...
      if (_.options.centerMode === true) {
        if (_.slideCount <= _.options.slidesToShow || _.options.infinite === false) {
          ...
          targetSlide = _.$slideTrack.children('.kick__slick-slide').eq(slideIndex + _.options.slidesToShow + 1);
        }
        if (_.options.rtl === true) {
          if (targetSlide[0]) {
            targetLeft = (_.$slideTrack.width() - targetSlide[0].offsetLeft - targetSlide.width()) * -1;
    ...
    var _ = this,
      ...
      _.$slideTrack.find('.kick__slick-slide').each(function (index, slide) {
    ...
    var _ = this;if (!$(_.$slider).hasClass('kick__slick-initialized')) {
      $(_.$slider).addClass('kick__slick-initialized');_.buildRows();_.buildOut();_.setProps();_.startLoad();_.loadSlider();_.initializeEvents();_.updateArrows();_.updateDots();_.checkResponsive(true);_.focusHandler();
    }
    if (creation) {
    ...
    });_.$slides.add(_.$slideTrack.find('.kick__slick-cloned')).attr({ 'aria-hidden': 'true', 'tabindex': '-1' }).find('a, input, button, select').attr({ 'tabindex': '-1' });if (_.$dots !== null) {
      _.$slides.not(_.$slideTrack.find('.kick__slick-cloned')).each(function (i) {
        var slideControlIndex = tabControlIndexes.indexOf(i);$(this).attr({ 'role': 'tabpanel', 'id': 'kick__slick-slide' + _.instanceUid + i, 'tabindex': -1 });if (slideControlIndex !== -1) {
          var ariaButtonControl = 'kick__slick-slide-control' + _.instanceUid + slideControlIndex;if ($('#' + ariaButtonControl).length) {
            $(this).attr({ 'aria-describedby': ariaButtonControl });
          }
        }
      });_.$dots.attr('role', 'tablist').find('li').each(function (i) {
        var mappedSlideIndex = tabControlIndexes[i];$(this).attr({ 'role': 'presentation' });$(this).find('button').first().attr({ 'role': 'tab', 'id': 'kick__slick-slide-control' + _.instanceUid + i, 'aria-controls': 'kick__slick-slide' + _.instanceUid + mappedSlideIndex, 'aria-label': i + 1 + ' of ' + numDotGroups, 'aria-selected': null, 'tabindex': '-1' });
    ...
    var _ = this;if (_.options.arrows === true && _.slideCount > _.options.slidesToShow) {
      _.$prevArrow.off('click.kick__slick').on('click.kick__slick', { message: 'previous' }, _.changeSlide);_.$nextArrow.off('click.kick__slick').on('click.kick__slick', { message: 'next' }, _.changeSlide);if (_.options.accessibility === true) {
        _.$prevArrow.on('keydown.kick__slick', _.keyHandler);_.$nextArrow.on('keydown.kick__slick', _.keyHandler);
      }
    }
  };Slick.prototype.initDotEvents = function () {
    var _ = this;if (_.options.dots === true && _.slideCount > _.options.slidesToShow) {
      $('li', _.$dots).on('click.kick__slick', { message: 'index' }, _.changeSlide);if (_.options.accessibility === true) {
        _.$dots.on('keydown.kick__slick', _.keyHandler);
      }
    }
    if (_.options.dots === true && _.options.pauseOnDotsHover === true && _.slideCount > _.options.slidesToShow) {
      $('li', _.$dots).on('mouseenter.kick__slick', $.proxy(_.interrupt, _, true)).on('mouseleave.kick__slick', $.proxy(_.interrupt, _, false));
    }
  };Slick.prototype.initSlideEvents = function () {
    var _ = this;if (_.options.pauseOnHover) {
      _.$list.on('mouseenter.kick__slick', $.proxy(_.interrupt, _, true));_.$list.on('mouseleave.kick__slick', $.proxy(_.interrupt, _, false));
    }
  };Slick.prototype.initializeEvents = function () {
    var _ = this;_.initArrowEvents();_.initDotEvents();_.initSlideEvents();_.$list.on('touchstart.kick__slick mousedown.kick__slick', { action: 'start' }, _.swipeHandler);_.$list.on('touchmove.kick__slick mousemove.kick__slick', { action: 'move' }, _.swipeHandler);_.$list.on('touchend.kick__slick mouseup.kick__slick', { action: 'end' }, _.swipeHandler);_.$list.on('touchcancel.kick__slick mouseleave.kick__slick', { action: 'end' }, _.swipeHandler);_.$list.on('click.kick__slick', _.clickHandler);$(document).on(_.visibilityChange, $.proxy(_.visibility, _));if (_.options.accessibility === true) {
      _.$list.on('keydown.slick', _.keyHandler);
    }
    if (_.options.focusOnSelect === true) {
      $(_.$slideTrack).children().on('click.slick', _.selectHandler);
    }
    $(window).on('orientationchange.slick.slick-' + _.instanceUid, $.proxy(_.orientationChange, _));$(window).on('resize.kick__slick.kick__slick-' + _.instanceUid, $.proxy(_.resize, _));$('[draggable!=true]', _.$slideTrack).on('dragstart', _.preventDefault);$(window).on('load.kick__slick.kick__slick-' + _.instanceUid, _.setPosition);$(_.setPosition);
      ...
      $('img[data-lazy]', imagesScope).each(function () {
        var image = $(this),
          ...
          image.animate({ opacity: 0 }, 100, function () {
            if (imageSrcSet) {
              image.attr('srcset', imageSrcSet);if (imageSizes) {
                image.attr('sizes', imageSizes);
              }
            }
            image.attr('src', imageSource).animate({ opacity: 1 }, 200, function () {
              image.removeAttr('data-lazy data-srcset data-sizes').removeClass('kick__slick-loading');
            });_.$slider.trigger('lazyLoaded', [_, image, imageSource]);
          });
        };imageToLoad.onerror = function () {
          image.removeAttr('data-lazy').removeClass('kick__slick-loading').addClass('kick__slick-lazyload-error');_.$slider.trigger('lazyLoadError', [_, image, imageSource]);
    ...
    loadRange = _.$slider.find('.kick__slick-slide').slice(rangeStart, rangeEnd);if (_.options.lazyLoad === 'anticipated') {
      var prevSlide = rangeStart - 1,
          nextSlide = rangeEnd,
          $slides = _.$slider.find('.kick__slick-slide');for (var i = 0; i < _.options.slidesToScroll; i++) {
        if (prevSlide < 0) prevSlide = _.slideCount - 1;loadRange = loadRange.add($slides.eq(prevSlide));loadRange = loadRange.add($slides.eq(nextSlide));prevSlide--;nextSlide++;
      }
    }
    loadImages(loadRange);if (_.slideCount <= _.options.slidesToShow) {
      cloneRange = _.$slider.find('.kick__slick-slide');loadImages(cloneRange);
    } else if (_.currentSlide >= _.slideCount - _.options.slidesToShow) {
      cloneRange = _.$slider.find('.kick__slick-cloned').slice(0, _.options.slidesToShow);loadImages(cloneRange);
    } else if (_.currentSlide === 0) {
      cloneRange = _.$slider.find('.kick__slick-cloned').slice(_.options.slidesToShow * -1);loadImages(cloneRange);
    }
  };Slick.prototype.loadSlider = function () {
    var _ = this;_.setPosition();_.$slideTrack.css({ opacity: 1 });_.$slider.removeClass('kick__slick-loading');_.initUI();if (_.options.lazyLoad === 'progressive') {
    ...
    var _ = this;if (!_.unslicked) {
      _.$slider.trigger('afterChange', [_, index]);_.animating = false;if (_.slideCount > _.options.slidesToShow) {
        _.setPosition();
      }
      ...
      if (_.options.accessibility === true) {
        _.initADA();if (_.options.focusOnChange) {
          var $currentSlide = $(_.$slides.get(_.currentSlide));$currentSlide.attr('tabindex', 0).focus();
        }
      }
    }
    ...
    tryCount = tryCount || 1;var _ = this,
      ...
      image = $imgsToLoad.first();imageSource = image.attr('data-lazy');imageSrcSet = image.attr('data-srcset');imageSizes = image.attr('data-sizes') || _.$slider.attr('data-sizes');imageToLoad = document.createElement('img');imageToLoad.onload = function () {
        ...
        image.attr('src', imageSource).removeAttr('data-lazy data-srcset data-sizes').removeClass('kick__slick-loading');if (_.options.adaptiveHeight === true) {
          _.setPosition();
        }
        ...
        if (tryCount < 3) {
          ...
          image.removeAttr('data-lazy').removeClass('kick__slick-loading').addClass('kick__slick-lazyload-error');_.$slider.trigger('lazyLoadError', [_, image, imageSource]);_.progressiveLazyLoad();
        }
      };imageToLoad.src = imageSource;
    } else {
    ...
    var _ = this;_.$slides = _.$slideTrack.children(_.options.slide).addClass('kick__slick-slide');_.slideCount = _.$slides.length;if (_.currentSlide >= _.slideCount && _.currentSlide !== 0) {
    ...
    _.registerBreakpoints();_.setProps();_.setupInfinite();_.buildArrows();_.updateArrows();_.initArrowEvents();_.buildDots();_.updateDots();_.initDotEvents();_.cleanUpSlideEvents();_.initSlideEvents();_.checkResponsive(false, true);if (_.options.focusOnSelect === true) {
      $(_.$slideTrack).children().on('click.kick__slick', _.selectHandler);
    }
    ...
    _.listWidth = _.$list.width();_.listHeight = _.$list.height();if (_.options.vertical === false && _.options.variableWidth === false) {
      _.slideWidth = Math.ceil(_.listWidth / _.options.slidesToShow);_.$slideTrack.width(Math.ceil(_.slideWidth * _.$slideTrack.children('.kick__slick-slide').length));
    } else if (_.options.variableWidth === true) {
      _.$slideTrack.width(5000 * _.slideCount);
    } else {
      _.slideWidth = Math.ceil(_.listWidth);_.$slideTrack.height(Math.ceil(_.$slides.first().outerHeight(true) * _.$slideTrack.children('.kick__slick-slide').length));
    }
    var offset = _.$slides.first().outerWidth(true) - _.$slides.first().width();if (_.options.variableWidth === false) _.$slideTrack.children('.kick__slick-slide').width(_.slideWidth - offset);
    ...
    var _ = this,
      ...
      _.$slider.addClass('kick__slick-vertical');
    } else {
      _.$slider.removeClass('kick__slick-vertical');
    }
    ...
    var _ = this,
        ...
        allSlides,
        indexOffset,
        remainder;allSlides = _.$slider.find('.kick__slick-slide').removeClass('kick__slick-active kick__slick-center kick__slick-current').attr('aria-hidden', 'true');_.$slides.eq(index).addClass('kick__slick-current');if (_.options.centerMode === true) {
      var evenCoef = _.options.slidesToShow % 2 === 0 ? 1 : 0;centerOffset = Math.floor(_.options.slidesToShow / 2);if (_.options.infinite === true) {
        if (index >= centerOffset && index <= _.slideCount - 1 - centerOffset) {
          _.$slides.slice(index - centerOffset + evenCoef, index + centerOffset + 1).addClass('kick__slick-active').attr('aria-hidden', 'false');
        } else {
          indexOffset = _.options.slidesToShow + index;allSlides.slice(indexOffset - centerOffset + 1 + evenCoef, indexOffset + centerOffset + 2).addClass('kick__slick-active').attr('aria-hidden', 'false');
        }
        if (index === 0) {
          allSlides.eq(allSlides.length - 1 - _.options.slidesToShow).addClass('kick__slick-center');
        } else if (index === _.slideCount - 1) {
          allSlides.eq(_.options.slidesToShow).addClass('kick__slick-center');
        }
      }
      _.$slides.eq(index).addClass('kick__slick-center');
    } else {
      if (index >= 0 && index <= _.slideCount - _.options.slidesToShow) {
        _.$slides.slice(index, index + _.options.slidesToShow).addClass('kick__slick-active').attr('aria-hidden', 'false');
      } else if (allSlides.length <= _.options.slidesToShow) {
        allSlides.addClass('kick__slick-active').attr('aria-hidden', 'false');
      } else {
        remainder = _.slideCount % _.options.slidesToShow;indexOffset = _.options.infinite === true ? _.options.slidesToShow + index : index;if (_.options.slidesToShow == _.options.slidesToScroll && _.slideCount - index < _.options.slidesToShow) {
          allSlides.slice(indexOffset - (_.options.slidesToShow - remainder), indexOffset + remainder).addClass('kick__slick-active').attr('aria-hidden', 'false');
        } else {
          allSlides.slice(indexOffset, indexOffset + _.options.slidesToShow).addClass('kick__slick-active').attr('aria-hidden', 'false');
        }
      }
    }
    ...
    if (_.options.infinite === true && _.options.fade === false) {
      slideIndex = null;if (_.slideCount > _.options.slidesToShow) {
        ...
        for (i = _.slideCount; i > _.slideCount - infiniteCount; i -= 1) {
          slideIndex = i - 1;$(_.$slides[slideIndex]).clone(true).attr('id', '').attr('data-slick-index', slideIndex - _.slideCount).prependTo(_.$slideTrack).addClass('kick__slick-cloned');
        }
        for (i = 0; i < infiniteCount + _.slideCount; i += 1) {
          slideIndex = i;$(_.$slides[slideIndex]).clone(true).attr('id', '').attr('data-slick-index', slideIndex + _.slideCount).appendTo(_.$slideTrack).addClass('kick__slick-cloned');
        }
        _.$slideTrack.find('.kick__slick-cloned').find('[id]').each(function () {
          $(this).attr('id', '');
        });
      }
    }
    ...
    var _ = this;var targetElement = $(event.target).is('.kick__slick-slide') ? $(event.target) : $(event.target).parents('.kick__slick-slide');var index = parseInt(targetElement.attr('data-slick-index'));if (!index) index = 0;if (_.slideCount <= _.options.slidesToShow) {
    ...
    targetSlide = index;targetLeft = _.getLeft(targetSlide);slideLeft = _.getLeft(_.currentSlide);_.currentLeft = _.swipeLeft === null ? slideLeft : _.swipeLeft;if (_.options.infinite === false && _.options.centerMode === false && (index < 0 || index > _.getDotCount() * _.options.slidesToScroll)) {
      if (_.options.fade === false) {
        targetSlide = _.currentSlide;if (dontAnimate !== true && _.slideCount > _.options.slidesToShow) {
    ...
    _.$slider.addClass('kick__slick-loading');
    ...
    var _ = this,
        slideCount,
        direction;_.dragging = false;_.swiping = false;if (_.scrolling) {
      _.scrolling = false;return false;
    }
    _.interrupted = false;_.shouldClick = _.touchObject.swipeLength > 10 ? false : true;if (_.touchObject.curX === undefined) {
      return false;
    }
    ...
    if (_.touchObject.swipeLength >= _.touchObject.minSwipe) {
      direction = _.swipeDirection();switch (direction) {case 'left':case 'down':
          ...
          slideCount = _.options.swipeToSlide ? _.checkNavigable(_.currentSlide - _.getSlideCount()) : _.currentSlide - _.getSlideCount();_.currentDirection = 1;break;default:}
    ...
    var _ = this,
        ...
        positionOffset,
        touches,
        verticalSwipeLength;touches = event.originalEvent !== undefined ? event.originalEvent.touches : null;if (!_.dragging || _.scrolling || touches && touches.length !== 1) {
      return false;
    }
    curLeft = _.getLeft(_.currentSlide);_.touchObject.curX = touches !== undefined ? touches[0].pageX : event.clientX;_.touchObject.curY = touches !== undefined ? touches[0].pageY : event.clientY;_.touchObject.swipeLength = Math.round(Math.sqrt(Math.pow(_.touchObject.curX - _.touchObject.startX, 2)));verticalSwipeLength = Math.round(Math.sqrt(Math.pow(_.touchObject.curY - _.touchObject.startY, 2)));if (!_.options.verticalSwiping && !_.swiping && verticalSwipeLength > 4) {
      _.scrolling = true;return false;
    }
    if (_.options.verticalSwiping === true) {
    ...
    var _ = this;$('.kick__slick-cloned', _.$slider).remove();if (_.$dots) {
      _.$dots.remove();
    }
    ...
    _.$slides.removeClass('kick__slick-slide kick__slick-active kick__slick-visible kick__slick-current').attr('aria-hidden', 'true').css('width', '');
    ...
    var _ = this;if (_.$dots !== null) {
      _.$dots.find('li').removeClass('kick__slick-active').end();_.$dots.find('li').eq(Math.floor(_.currentSlide / _.options.slidesToScroll)).addClass('kick__slick-active');
    }
    ...
    var _ = this,
        ...
        i,
        ret;for (i = 0; i < l; i++) {
      if (typeof opt == 'object' || typeof opt == 'undefined') _[i].slick = new Slick(_[i], opt);else ret = _[i].slick[opt].apply(_[i].slick, args);if (typeof ret != 'undefined') return ret;
    }
    return _;
  };
});